...
@Component({
      ...
      <div class="change-card">
        ...
        <div class="brand">
          <div class="logo-wrapper">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="-1.2380214 -1.2380214 357.6729128 43.7434228" class="logo" preserveAspectRatio="xMidYMid meet" aria-label="Hutchinson">
              <path
          ...
          <p class="tagline">{{ 'CHANGE_PASSWORD.TAGLINE' | translate }}</p>
        </div>

        <div class="form-container">
          <h2 class="form-title">{{ 'CHANGE_PASSWORD.TITLE' | translate }}</h2>
          <p class="info-message">{{ 'CHANGE_PASSWORD.SUBTITLE' | translate }}</p>
          ...
          <form [formGroup]="passwordForm" (ngSubmit)="onSubmit()" novalidate>
            ...
            <div class="input-group" [class.focused]="newPwdFocused">
              <label for="newPassword">
                ...
                {{ 'CHANGE_PASSWORD.NEW_PASSWORD_LABEL' | translate }}
              </label>
              <div class="password-wrapper">
                <input
                  ...
                  [placeholder]="'CHANGE_PASSWORD.PASSWORD_PLACEHOLDER' | translate"
              ...
              <div class="error-msg" *ngIf="passwordForm.get('newPassword')?.invalid && passwordForm.get('newPassword')?.touched">
                <span *ngIf="passwordForm.get('newPassword')?.errors?.['required']">⚠️ {{ 'CHANGE_PASSWORD.PASSWORD_REQUIRED' | translate }}</span>
                <span *ngIf="passwordForm.get('newPassword')?.errors?.['minlength']">⚠️ {{ 'CHANGE_PASSWORD.PASSWORD_MINLENGTH' | translate }}</span>
              </div>
            </div>
            ...
            <div class="input-group" [class.focused]="confirmPwdFocused">
              <label for="confirmPassword">
                ...
                {{ 'CHANGE_PASSWORD.CONFIRM_LABEL' | translate }}
              </label>
              <div class="password-wrapper">
                <input
                  id="confirmPassword"
              ...
              <div class="error-msg" *ngIf="passwordForm.hasError('mismatch') && passwordForm.get('confirmPassword')?.touched">
                ⚠️ {{ 'CHANGE_PASSWORD.PASSWORD_MISMATCH' | translate }}
              </div>
            </div>
            ...
            <button type="submit" class="submit-btn" [disabled]="passwordForm.invalid || isSubmitting">
              <span class="btn-text">{{ isSubmitting ? ('CHANGE_PASSWORD.SUBMITTING' | translate) : ('CHANGE_PASSWORD.SUBMIT_BUTTON' | translate) }}</span>
          ...
          <div class="footer-note">
            <span class="dot"></span> {{ 'CHANGE_PASSWORD.FOOTER_TEXT' | translate }} <span class="dot"></span>  © {{ currentYear }}
          </div>
        </div>
      </div>
    </div>
  `,
  styles: [`
    /* ===== ULTRA PRO STYLES (inchangés sauf bouton toggle) ===== */
    * {
      margin: 0;
      padding: 0;
    ...
    .animated-gradient {
      ...
      z-index: 1;
      animation: shift 14s infinite alternate;
    }
...
export class ChangePasswordComponent implements OnInit {
  ...
  getPasswordStrengthText(): string {
    const pwd = this.passwordForm.get('newPassword')?.value || '';
    const strength = this.checkStrength(pwd);
    if (strength === 'weak') return this.translate.instant('CHANGE_PASSWORD.STRENGTH_WEAK');
    if (strength === 'medium') return this.translate.instant('CHANGE_PASSWORD.STRENGTH_MEDIUM');
    if (strength === 'strong') return this.translate.instant('CHANGE_PASSWORD.STRENGTH_STRONG');
    return '';
  }
  ...
  onSubmit(): void {
    ...
    const request = { newPassword: this.passwordForm.value.newPassword };
    this.authService.changePassword(request).subscribe({
      ...
      error: (err) => {
        ...
        this.errorMessage = err.error?.message || this.translate.instant('CHANGE_PASSWORD.ERROR_MESSAGE');
      }
    });
  }
}