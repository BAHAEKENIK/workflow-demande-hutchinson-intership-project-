...
@Component({
  ...
  template: `
    <div class="forgot-container">
      <!-- Background avec image en ligne, flou léger mais identifiable -->
      ...
      <div class="forgot-card">
        ...
        <div class="brand">
          ...
          <p class="tagline">{{ 'FORGOT_PASSWORD.TAGLINE' | translate }}</p>
        </div>

        <!-- Form -->
        <div class="form-container">
          <h2 class="form-title">{{ 'FORGOT_PASSWORD.TITLE' | translate }}</h2>
          ...
          <form [formGroup]="emailForm" (ngSubmit)="onSubmit()" novalidate>
            <div class="input-group" [class.focused]="emailFocused">
              ...
              <input
                ...
                autocomplete="email"
              />
              <span class="error-msg" *ngIf="emailForm.get('email')?.invalid && emailForm.get('email')?.touched">
                <span class="error-icon">⚠️</span> {{ 'FORGOT_PASSWORD.EMAIL_REQUIRED' | translate }}
              </span>
            </div>
            ...
            <button type="submit" class="submit-btn" [disabled]="emailForm.invalid || isSubmitting">
              <span class="btn-text">{{ isSubmitting ? ('FORGOT_PASSWORD.SENDING' | translate) : ('FORGOT_PASSWORD.SUBMIT_BUTTON' | translate) }}</span>
          ...
          <div class="footer-note">
            <span class="dot"></span> {{ 'FORGOT_PASSWORD.FOOTER_TEXT' | translate }}   © {{ currentYear }}
          </div>
        </div>
      </div>
    </div>
  ...
  styles: [`
    * {
    ...
    /* Background layers - image floue mais visible */
    .bg-blur {
      position: absolute;
      inset: 0;
    ...
    .forgot-card {
      ...
      box-shadow: 0 25px 45px rgba(0, 0, 0, 0.2), 0 0 0 1px rgba(255, 255, 255, 0.7) inset;
    ...
    this.authService.forgotPassword(this.emailForm.value.email).subscribe({
      ...
      error: (err) => {
        ...
        this.errorMessage = err.error?.message ||
          this.translate.instant('FORGOT_PASSWORD.ERROR_MESSAGE');
      }
    });
  }
}