package com.workflow.service;

import com.workflow.dto.request.ValidationRequest;
import com.workflow.entity.*;
import com.workflow.repository.DemandeRepository;
import com.workflow.repository.EtapeValidationRepository;
import com.workflow.repository.HistoriqueRepository;
import com.workflow.repository.UserRepository;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;
import org.springframework.security.access.AccessDeniedException;
import org.springframework.stereotype.Service;
import org.springframework.transaction.annotation.Transactional;

import java.util.HashMap;
import java.util.List;
import java.util.Map;

@Service
public class ValidationService {

    private static final Logger logger = LoggerFactory.getLogger(ValidationService.class);

    private final DemandeRepository demandeRepository;
    private final EtapeValidationRepository etapeRepository;
    private final HistoriqueRepository historiqueRepository;
    private final UserService userService;
    private final EmailService emailService;
    private final PdfService pdfService;
    private final UserRepository userRepository;
    private final NotificationService notificationService;
    private final DemandeService demandeService;

    public ValidationService(DemandeRepository demandeRepository,
                             EtapeValidationRepository etapeRepository,
                             HistoriqueRepository historiqueRepository,
                             UserService userService,
                             EmailService emailService,
                             PdfService pdfService,
                             UserRepository userRepository,
                             NotificationService notificationService,
                             DemandeService demandeService) {
        this.demandeRepository = demandeRepository;
        this.etapeRepository = etapeRepository;
        this.historiqueRepository = historiqueRepository;
        this.userService = userService;
        this.emailService = emailService;
        this.pdfService = pdfService;
        this.userRepository = userRepository;
        this.notificationService = notificationService;
        this.demandeService = demandeService;
    }

    @Transactional
    public void validerDemande(ValidationRequest request) {

        Demande demande = demandeRepository.findByIdWithWorkflowEtapes(request.getDemandeId())
                .orElseThrow(() -> new RuntimeException("Demande non trouvée"));

        User validateur = userService.getCurrentUser();

        if (demande.getStatut() != StatutDemande.PENDING) {
            throw new IllegalStateException("Cette demande n'est plus en attente de validation");
        }

        int currentIndex = demande.getEtapeCourante();
        List<EtapeValidation> etapes = demande.getWorkflow().getEtapes();

        if (currentIndex >= etapes.size()) {
            throw new IllegalStateException("Aucune étape en attente");
        }

        EtapeValidation etape = etapes.get(currentIndex);

        User expectedValidator = getValidatorUser(etape);
        if (expectedValidator == null) {
            throw new IllegalStateException("Aucun validateur défini pour cette étape");
        }

        if (!expectedValidator.getId().equals(validateur.getId())) {
            String expectedName = expectedValidator.getFirstName() + " " + expectedValidator.getLastName();
            throw new AccessDeniedException("Seul " + expectedName + " peut valider cette étape");
        }

        Historique histo = new Historique();
        histo.setDemande(demande);
        histo.setUtilisateur(validateur);
        histo.setAction(request.getAction());
        histo.setCommentaire(request.getCommentaire());
        histo.setEtapeIndex(currentIndex);
        historiqueRepository.save(histo);

        if ("REJECT".equalsIgnoreCase(request.getAction())) {
            if (request.getCommentaire() == null || request.getCommentaire().isBlank()) {
                throw new IllegalArgumentException("Un commentaire est obligatoire pour un rejet");
            }
            etape.setStatut(StatutEtape.REJECTED);
            etapeRepository.save(etape);
            demande.setStatut(StatutDemande.REJECTED);
            demandeRepository.save(demande);

            String demandeurHtml = buildHtmlEmailTemplate(
                    "Demande rejetée",
                    "Bonjour " + demande.getDemandeur().getFirstName() + ",",
                    "Votre demande \"" + demande.getTitre() + "\" a été rejetée par " + validateur.getUsername() + ".",
                    Map.of("Commentaire", request.getCommentaire()),
                    null, null, false,
                    "Veuillez contacter votre responsable pour plus d'informations."
            );
            try {
                emailService.sendHtmlEmail(demande.getDemandeur().getEmail(), "Demande rejetée", demandeurHtml);
            } catch (Exception e) {
                logger.error("Échec envoi email rejet au demandeur (ID demande {})", demande.getId(), e);
            }

            User createur = demande.getCreateur();
            String createurHtml = buildHtmlEmailTemplate(
                    "❌ Demande rejetée – " + demande.getTitre(),
                    "Bonjour " + createur.getFirstName() + ",",
                    "La demande \"" + demande.getTitre() + "\" que vous avez créée a été rejetée par " + validateur.getFirstName() + " " + validateur.getLastName() + " à l'étape " + (currentIndex + 1) + ".",
                    Map.of("Commentaire", request.getCommentaire()),
                    null, null, false,
                    "Cordialement, L'équipe Hutchinson"
            );
            try {
                emailService.sendHtmlEmail(createur.getEmail(), "❌ Demande rejetée – " + demande.getTitre(), createurHtml);
            } catch (Exception e) {
                logger.error("Échec envoi email rejet au créateur (ID demande {})", demande.getId(), e);
            }

            notificationService.createNotification(
                    createur,
                    "❌ Demande rejetée",
                    "Votre demande \"" + demande.getTitre() + "\" a été rejetée par " + validateur.getFirstName() + " " + validateur.getLastName() + ".\nCommentaire : " + request.getCommentaire(),
                    "/demandes/" + demande.getId()
            );

            return;
        }

        if ("APPROVE".equalsIgnoreCase(request.getAction())) {
            etape.setStatut(StatutEtape.APPROVED);
            etapeRepository.save(etape);

            int nextIndex = currentIndex + 1;

            if (nextIndex >= etapes.size()) {
                demande.setStatut(StatutDemande.APPROVED);
                demande.setEtapeCourante(etapes.size());
                demandeRepository.save(demande);

                if (demande.getTitre().startsWith("SUPPRESSION_")) {
                    String[] parts = demande.getTitre().split("_");
                    if (parts.length >= 2) {
                        String username = parts[1];
                        User toDelete = userRepository.findByUsername(username).orElse(null);
                        if (toDelete != null) {
                            Map<String, String> userInfo = new HashMap<>();
                            userInfo.put("username", toDelete.getUsername());
                            userInfo.put("role", toDelete.getRole().name());
                            userInfo.put("firstName", toDelete.getFirstName());
                            userInfo.put("lastName", toDelete.getLastName());
                            userInfo.put("email", toDelete.getEmail());

                            Department dept = toDelete.getDepartment();
                            if (dept != null) {
                                userInfo.put("department", dept.getName());
                                userInfo.put("departmentLocal", dept.isLocal() ? "Oui" : "Non");
                                if (dept.getChef() != null) {
                                    userInfo.put("responsable", dept.getChef().getFirstName() + " " + dept.getChef().getLastName());
                                    userInfo.put("responsableEmail", dept.getChef().getEmail());
                                } else {
                                    userInfo.put("responsable", "Non défini");
                                    userInfo.put("responsableEmail", "");
                                }
                                if (!dept.isLocal() && dept.getSecondChef() != null) {
                                    userInfo.put("secondResponsable", dept.getSecondChef().getFirstName() + " " + dept.getSecondChef().getLastName());
                                    userInfo.put("secondResponsableEmail", dept.getSecondChef().getEmail());
                                } else {
                                    userInfo.put("secondResponsable", "");
                                    userInfo.put("secondResponsableEmail", "");
                                }
                            } else {
                                userInfo.put("department", "Aucun");
                                userInfo.put("responsable", "Non défini");
                                userInfo.put("responsableEmail", "");
                            }

                            userService.deleteUserTransactional(toDelete.getId());

                            String deletedUserHtml = buildHtmlEmailTemplate(
                                    "🗑️ Votre compte a été supprimé",
                                    "Bonjour " + toDelete.getFirstName() + ",",
                                    "Votre compte a été supprimé du système par demande administrative.",
                                    null, null, null, false,
                                    "Cordialement, L'équipe Hutchinson"
                            );
                            try {
                                emailService.sendHtmlEmail(toDelete.getEmail(), "🗑️ Votre compte a été supprimé", deletedUserHtml);
                            } catch (Exception e) {
                                logger.error("Échec envoi email suppression à l'utilisateur {}", toDelete.getUsername(), e);
                            }

                            String lang = "fr";
                            byte[] pdfBytes = pdfService.generateSuppressionPdf(demande, userInfo, lang);
                            String filename = "suppression_" + toDelete.getUsername() + "_" + demande.getId() + ".pdf";

                            List<User> admins = userRepository.findAllActiveAdmins();
                            for (User admin : admins) {
                                String adminBody = buildHtmlEmailTemplate(
                                        "✅ Suppression validée - Détails de l'utilisateur supprimé",
                                        "Bonjour " + admin.getFirstName() + ",",
                                        "La suppression de l'utilisateur " + toDelete.getUsername() + " (" + toDelete.getFirstName() + " " + toDelete.getLastName() + ") a été approuvée.",
                                        Map.of("Utilisateur supprimé", toDelete.getUsername() + " (" + toDelete.getFirstName() + " " + toDelete.getLastName() + ")"),
                                        "Voir le détail", "/admin/users", true,
                                        "Vous trouverez ci-joint le récapitulatif complet."
                                );
                                try {
                                    emailService.sendEmailWithAttachment(
                                            admin.getEmail(),
                                            "✅ Suppression validée - Détails de l'utilisateur supprimé",
                                            adminBody,
                                            pdfBytes,
                                            filename
                                    );
                                } catch (Exception e) {
                                    logger.error("Échec envoi email à l'admin {} pour la suppression de {}", admin.getEmail(), toDelete.getUsername(), e);
                                }
                                notificationService.createNotification(
                                        admin,
                                        "✅ Suppression validée",
                                        "L'utilisateur " + toDelete.getUsername() + " a été supprimé avec succès. Un email récapitulatif vous a été envoyé.",
                                        "/admin/users"
                                );
                            }
                        }
                    }
                } else {
                    String lang = "fr";
                    byte[] pdfSimple = pdfService.generateFullPdf(demande, lang);
                    String filenameSimple = "demande_" + demande.getId() + "_approuvee.pdf";
                    String demandeurBody = buildHtmlEmailTemplate(
                            "📄 Récapitulatif de votre demande approuvée",
                            "Bonjour " + demande.getDemandeur().getFirstName() + ",",
                            "Nous avons le plaisir de vous informer que votre demande \"" + demande.getTitre() + "\" a été approuvée.",
                            null, "Voir la demande", "/demandes/" + demande.getId(), true,
                            "Vous trouverez ci-joint le récapitulatif détaillé."
                    );
                    try {
                        emailService.sendEmailWithAttachment(
                                demande.getDemandeur().getEmail(),
                                "📄 Récapitulatif de votre demande approuvée",
                                demandeurBody,
                                pdfSimple,
                                filenameSimple
                        );
                    } catch (Exception e) {
                        logger.error("Échec envoi PDF simple au demandeur (ID demande {})", demande.getId(), e);
                    }

                    User employee = demande.getDemandeur();
                    List<Demande> previousDemandes = demandeService.getPreviousApprovedDemandesForEmployee(employee, demande.getId());

                    byte[] pdfWithHistory = pdfService.generateFullPdfWithHistory(demande, previousDemandes, lang);
                    String filenameHistory = "besoins_employe_" + employee.getId() + "_" + demande.getId() + ".pdf";

                    List<User> admins = userRepository.findAllActiveAdmins();
                    for (User admin : admins) {
                        String adminBody = buildHtmlEmailTemplate(
                                "📋 Nouvelle hoja à créer pour " + employee.getFirstName() + " " + employee.getLastName(),
                                "Bonjour " + admin.getFirstName() + ",",
                                "Une demande de besoins pour l'employé " + employee.getFirstName() + " " + employee.getLastName() + " a été approuvée.",
                                null, "Créer la hoja", "/admin/users?employeeId=" + employee.getId(), true,
                                "Vous trouverez ci-joint le récapitulatif complet (anciens + nouveaux besoins). Merci de créer la hoja correspondante."
                        );
                        try {
                            emailService.sendEmailWithAttachment(
                                    admin.getEmail(),
                                    "📋 Nouvelle hoja à créer pour " + employee.getFirstName() + " " + employee.getLastName(),
                                    adminBody,
                                    pdfWithHistory,
                                    filenameHistory
                            );
                        } catch (Exception e) {
                            logger.error("Échec envoi PDF avec historique à l'admin {}", admin.getEmail(), e);
                        }
                        notificationService.createNotification(
                                admin,
                                "📄 Création de hoja requise",
                                "Une demande de besoins pour " + employee.getFirstName() + " " + employee.getLastName() + " a été approuvée. Veuillez créer la hoja.",
                                "/admin/users?employeeId=" + employee.getId()
                        );
                    }

                    User chefDept = demande.getCreateur();
                    String chefBody = buildHtmlEmailTemplate(
                            "✅ Demande de besoins acceptée",
                            "Bonjour " + chefDept.getFirstName() + ",",
                            "La demande de besoins que vous avez saisie pour " + employee.getFirstName() + " " + employee.getLastName() + " a été approuvée par toutes les étapes.",
                            null, "Voir la demande", "/demandes/" + demande.getId(), true,
                            "Vous trouverez ci-joint le récapitulatif complet."
                    );
                    try {
                        emailService.sendEmailWithAttachment(
                                chefDept.getEmail(),
                                "✅ Demande de besoins acceptée",
                                chefBody,
                                pdfWithHistory,
                                filenameHistory
                        );
                    } catch (Exception e) {
                        logger.error("Échec envoi PDF avec historique au chef de département (ID demande {})", demande.getId(), e);
                    }
                    notificationService.createNotification(
                            chefDept,
                            "✅ Demande approuvée",
                            "Votre demande de besoins pour " + employee.getFirstName() + " " + employee.getLastName() + " a été acceptée. Un récapitulatif vous a été envoyé par email.",
                            "/demandes/" + demande.getId()
                    );
                }
            } else {
                demande.setEtapeCourante(nextIndex);
                demandeRepository.save(demande);

                User createur = demande.getCreateur();
                String commentaire = request.getCommentaire() != null ? request.getCommentaire() : "Aucun commentaire";
                String stepApprovalHtml = buildHtmlEmailTemplate(
                        "✅ Étape approuvée – " + demande.getTitre(),
                        "Bonjour " + createur.getFirstName() + ",",
                        "L'étape " + (currentIndex + 1) + " de la demande \"" + demande.getTitre() + "\" a été approuvée par " + validateur.getFirstName() + " " + validateur.getLastName() + ".",
                        Map.of("Commentaire", commentaire),
                        "Voir la demande", "/demandes/" + demande.getId(), true,
                        "La demande passe maintenant à l'étape suivante."
                );
                try {
                    emailService.sendHtmlEmail(createur.getEmail(), "✅ Étape approuvée – " + demande.getTitre(), stepApprovalHtml);
                } catch (Exception e) {
                    logger.error("Échec envoi email étape approuvée au créateur (ID demande {})", demande.getId(), e);
                }
                notificationService.createNotification(
                        createur,
                        "✅ Étape approuvée",
                        "L'étape " + (currentIndex + 1) + " de votre demande \"" + demande.getTitre() + "\" a été approuvée par " + validateur.getFirstName() + " " + validateur.getLastName() + ".",
                        "/demandes/" + demande.getId()
                );

                EtapeValidation nextEtape = etapes.get(nextIndex);
                User nextValidator = getValidatorUser(nextEtape);
                if (nextValidator != null) {
                    notificationService.createNotification(
                            nextValidator,
                            "Nouvelle demande à valider",
                            "Une demande \"" + demande.getTitre() + "\" vous attend.",
                            "/demandes/" + demande.getId()
                    );
                }

                String nextEmail = getEmailForEtape(nextEtape);
                if (nextEmail != null) {
                    String baseUrl = emailService.getBaseUrl();
                    String demandeUrl = baseUrl + "/demandes/" + demande.getId();

                    String nextValidatorHtml = buildHtmlEmailTemplate(
                            "📌 Nouvelle demande à valider",
                            "Bonjour " + nextValidator.getFirstName() + ",",
                            "Une demande a été approuvée à l'étape précédente et a besoin de votre validation.",
                            Map.of(
                                    "Titre", demande.getTitre(),
                                    "Demandeur", demande.getDemandeur().getFirstName() + " " + demande.getDemandeur().getLastName(),
                                    "Département", (demande.getDemandeur().getDepartment() != null ? demande.getDemandeur().getDepartment().getName() : "Non spécifié")
                            ),
                            "🔍 Voir la demande", demandeUrl, true,
                            "Merci de vous connecter pour examiner et traiter cette demande."
                    );
                    try {
                        emailService.sendHtmlEmail(nextEmail, "📌 Nouvelle demande à valider", nextValidatorHtml);
                    } catch (Exception e) {
                        logger.error("Échec envoi email au validateur suivant ({}) pour la demande {}", nextEmail, demande.getId(), e);
                    }
                }
            }
        }
    }

    /**
     * Détermine le validateur pour une étape donnée.
     * Retourne uniquement le chef du département (ou second chef pour les départements externes)
     * afin que la validation ne soit effectuée que par la personne explicitement désignée.
     */
    private User getValidatorUser(EtapeValidation etape) {
        if (etape == null || etape.getDepartment() == null) return null;
        Department dept = etape.getDepartment();
        // Priorité au chef principal
        if (dept.getChef() != null) return dept.getChef();
        // Pour les départements externes, second chef
        if (!dept.isLocal() && dept.getSecondChef() != null) return dept.getSecondChef();
        return null;
    }

    private String getEmailForEtape(EtapeValidation etape) {
        if (etape.getDepartment() != null) {
            Department dept = etape.getDepartment();
            if (dept.getChef() != null) return dept.getChef().getEmail();
            if (!dept.isLocal() && dept.getSecondChef() != null) return dept.getSecondChef().getEmail();
        }
        return null;
    }

    private String buildHtmlEmailTemplate(String title, String greeting, String mainMessage,
                                          Map<String, String> details, String buttonText,
                                          String buttonUrl, boolean includeButton, String footerNote) {
        StringBuilder detailsHtml = new StringBuilder();
        if (details != null && !details.isEmpty()) {
            detailsHtml.append("<div class='details-card'>");
            for (Map.Entry<String, String> entry : details.entrySet()) {
                detailsHtml.append("<div class='detail-row'>")
                        .append("<span class='detail-label'>").append(entry.getKey()).append("</span>")
                        .append("<span class='detail-value'>").append(entry.getValue()).append("</span>")
                        .append("</div>");
            }
            detailsHtml.append("</div>");
        }

        String buttonHtml = "";
        if (includeButton && buttonText != null && buttonUrl != null) {
            buttonHtml = "<div style='text-align: center; margin: 28px 0 8px;'>"
                    + "<a href='" + buttonUrl + "' class='cta-button'>" + buttonText + "</a>"
                    + "</div>";
        }

        String footerHtml = (footerNote != null
                ? "<div class='footer-note'>" + footerNote + "</div>"
                : "");

        String htmlTemplate = """
                <!DOCTYPE html>
                <html lang="fr">
                <head>
                <meta charset="UTF-8">
                <meta name="viewport" content="width=device-width, initial-scale=1.0">
                <meta http-equiv="X-UA-Compatible" content="IE=edge">
                <title>%s</title>
                <style>
                  /* ─── RESET & GLOBAL ─── */
                  body, table, td, a { -webkit-text-size-adjust: 100%%; -ms-text-size-adjust: 100%%; }
                  table, td { mso-table-lspace: 0pt; mso-table-rspace: 0pt; }
                  img { -ms-interpolation-mode: bicubic; border: 0; height: auto; line-height: 100%%; outline: none; text-decoration: none; }
                  body { margin: 0; padding: 0; width: 100%% !important; height: 100%% !important; background-color: #e8ecf1; font-family: 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; }

                  /* ─── OUTER WRAPPER ─── */
                  .email-bg { background-color: #e8ecf1; padding: 40px 16px; min-height: 100vh; }

                  /* ─── MAIN CONTAINER ─── */
                  .container { max-width: 640px; margin: 0 auto; background: #ffffff; border-radius: 20px; overflow: hidden; box-shadow: 0 20px 60px rgba(15,23,42,0.12), 0 4px 16px rgba(15,23,42,0.06); }

                  /* ─── HEADER ─── */
                  .header {
                    background: linear-gradient(145deg, #0b1a3e 0%%, #1a2d5e 40%%, #243b6e 70%%, #1a2d5e 100%%);
                    padding: 40px 36px 32px;
                    text-align: center;
                    position: relative;
                    overflow: hidden;
                  }
                  .header::before {
                    content: '';
                    position: absolute;
                    top: -50%%;
                    left: -50%%;
                    width: 200%%;
                    height: 200%%;
                    background: radial-gradient(ellipse at 30%% 20%%, rgba(226,28,42,0.08) 0%%, transparent 60%%);
                    pointer-events: none;
                  }
                  .header::after {
                    content: '';
                    position: absolute;
                    bottom: 0;
                    left: 0;
                    right: 0;
                    height: 4px;
                    background: linear-gradient(90deg, #E21C2A 0%%, #ff4d5a 50%%, #E21C2A 100%%);
                  }
                  .logo-wrapper { margin-bottom: 20px; text-align: center; position: relative; z-index: 1; }
                  .logo-wrapper svg { max-width: 200px; height: auto; filter: brightness(1.1); }
                  .header h1 {
                    color: #ffffff;
                    font-size: 22px;
                    font-weight: 700;
                    margin: 0;
                    letter-spacing: -0.3px;
                    line-height: 1.4;
                    position: relative;
                    z-index: 1;
                  }
                  .header-accent {
                    display: inline-block;
                    width: 50px;
                    height: 3px;
                    background: linear-gradient(90deg, #E21C2A, #ff6b75);
                    border-radius: 2px;
                    margin-top: 14px;
                  }

                  /* ─── CONTENT ─── */
                  .content { padding: 40px 36px 32px; }
                  .greeting {
                    font-size: 20px;
                    font-weight: 700;
                    color: #0b1a3e;
                    margin-bottom: 12px;
                    letter-spacing: -0.2px;
                  }
                  .message {
                    color: #4a5568;
                    font-size: 15px;
                    line-height: 1.7;
                    margin-bottom: 28px;
                  }

                  /* ─── DETAILS CARD ─── */
                  .details-card {
                    background: linear-gradient(135deg, #f8fafc 0%%, #f1f5f9 100%%);
                    border: 1px solid #e2e8f0;
                    border-left: 4px solid #E21C2A;
                    border-radius: 14px;
                    padding: 20px 24px;
                    margin: 24px 0;
                  }
                  .detail-row {
                    padding: 10px 0;
                    border-bottom: 1px solid #edf2f7;
                    display: flex;
                    justify-content: space-between;
                    align-items: baseline;
                  }
                  .detail-row:last-child { border-bottom: none; }
                  .detail-label {
                    color: #64748b;
                    font-size: 13px;
                    font-weight: 600;
                    text-transform: uppercase;
                    letter-spacing: 0.5px;
                    flex-shrink: 0;
                    padding-right: 16px;
                  }
                  .detail-value {
                    color: #1e293b;
                    font-size: 14px;
                    font-weight: 500;
                    text-align: right;
                  }

                  /* ─── CTA BUTTON ─── */
                  .cta-button {
                    display: inline-block;
                    background: linear-gradient(135deg, #E21C2A 0%%, #c41924 100%%);
                    color: #ffffff !important;
                    text-decoration: none;
                    font-weight: 700;
                    font-size: 15px;
                    letter-spacing: 0.3px;
                    padding: 15px 40px;
                    border-radius: 50px;
                    box-shadow: 0 6px 20px rgba(226,28,42,0.35), 0 2px 6px rgba(226,28,42,0.2);
                    transition: all 0.25s ease;
                    mso-padding-alt: 0;
                    text-align: center;
                  }
                  .cta-button:hover {
                    background: linear-gradient(135deg, #c41924 0%%, #a8141d 100%%);
                    box-shadow: 0 8px 28px rgba(226,28,42,0.45), 0 4px 10px rgba(226,28,42,0.25);
                    transform: translateY(-2px);
                  }

                  /* ─── FOOTER NOTE ─── */
                  .footer-note {
                    margin-top: 28px;
                    padding: 16px 20px;
                    background: linear-gradient(135deg, #fef9f0 0%%, #fef3e2 100%%);
                    border-radius: 12px;
                    border: 1px solid #fde68a;
                    color: #92400e;
                    font-size: 13px;
                    line-height: 1.6;
                    text-align: center;
                    font-weight: 500;
                  }

                  /* ─── FOOTER ─── */
                  .footer {
                    background: linear-gradient(180deg, #f8fafc 0%%, #f1f5f9 100%%);
                    padding: 28px 36px;
                    text-align: center;
                    border-top: 1px solid #e2e8f0;
                  }
                  .footer-brand {
                    font-size: 14px;
                    font-weight: 700;
                    color: #0b1a3e;
                    letter-spacing: 0.5px;
                    margin-bottom: 6px;
                  }
                  .footer-text {
                    font-size: 12px;
                    color: #94a3b8;
                    line-height: 1.6;
                  }
                  .footer-text a {
                    color: #E21C2A;
                    text-decoration: none;
                    font-weight: 600;
                    border-bottom: 1px solid rgba(226,28,42,0.3);
                    transition: border-color 0.2s;
                  }
                  .footer-text a:hover {
                    border-bottom-color: #E21C2A;
                  }
                  .footer-divider {
                    width: 40px;
                    height: 2px;
                    background: linear-gradient(90deg, #E21C2A, #ff6b75);
                    border-radius: 1px;
                    margin: 14px auto;
                  }

                  /* ─── RESPONSIVE (MOBILE) ─── */
                  @media only screen and (max-width: 600px) {
                    .email-bg { padding: 16px 8px; }
                    .container { border-radius: 14px; }
                    .header { padding: 28px 20px 24px; }
                    .content { padding: 28px 20px 24px; }
                    .header h1 { font-size: 19px; }
                    .greeting { font-size: 18px; }
                    .cta-button { padding: 14px 32px; font-size: 14px; }
                    .detail-row { flex-direction: column; }
                    .detail-label { margin-bottom: 4px; }
                    .detail-value { text-align: left; }
                  }
                </style>
                </head>
                <body>
                <div class="email-bg">
                <div class='container'>

                  <!-- HEADER -->
                  <div class='header'>
                    <div class='logo-wrapper'>
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        viewBox="-1.2380214 -1.2380214 357.6729128 43.7434228"
                        class="logo"
                        preserveAspectRatio="xMidYMid meet"
                        aria-label="Hutchinson"
                      >
                        <path
                          d="m 350.15339,11.80675 0,1.49625 0.685,0 c 0.39125,0 0.67499,-0.0563 0.85124,-0.17125 0.18251,-0.1175 0.27,-0.30375 0.27,-0.55375 0,-0.26625 -0.0913,-0.46 -0.28,-0.58625 -0.1875,-0.12375 -0.48124,-0.185 -0.88999,-0.185 l -0.63625,0 z m -1.04001,-0.64375 1.86375,0 c 0.6725,0 1.185,0.11 1.53125,0.3275 0.3475,0.21625 0.52124,0.54125 0.52124,0.96625 0,0.3475 -0.095,0.63625 -0.28125,0.86375 -0.1875,0.2275 -0.45125,0.3775 -0.79875,0.45 l 1.08,2.15875 -1.16124,0 -0.9625,-1.99 -0.7525,0 0,1.99 -1.04,0 0,-4.76625 z m 1.765,-1.14875 c -0.50126,0 -0.96375,0.0863 -1.39625,0.25875 -0.43251,0.17625 -0.8175,0.43375 -1.16251,0.77125 -0.35624,0.3575 -0.63375,0.7625 -0.82499,1.20375 -0.19251,0.445 -0.28751,0.905 -0.28751,1.38375 0,0.4775 0.0913,0.9325 0.27501,1.36625 0.17999,0.43125 0.44499,0.8225 0.78749,1.16625 0.35001,0.3475 0.7475,0.61625 1.18875,0.8025 0.445,0.18625 0.89625,0.2775 1.36251,0.2775 0.50374,0 0.98124,-0.0888 1.43375,-0.2675 0.45249,-0.17875 0.85999,-0.4425 1.21875,-0.78875 0.3475,-0.3325 0.61124,-0.71 0.79499,-1.14 0.19,-0.42875 0.28251,-0.88125 0.28251,-1.36 0,-0.5075 -0.0888,-0.9825 -0.26499,-1.4175 -0.17377,-0.43875 -0.43751,-0.83125 -0.78125,-1.17875 -0.36375,-0.35125 -0.76376,-0.62 -1.20751,-0.8025 -0.445,-0.1825 -0.91749,-0.275 -1.41875,-0.275 m -0.004,-0.63625 c 0.59249,0 1.15499,0.105 1.68125,0.32125 0.5275,0.2125 0.99875,0.52875 1.41001,0.9475 0.39875,0.3975 0.70875,0.85375 0.91625,1.36125 0.20999,0.50625 0.31499,1.0425 0.31499,1.61625 0,0.585 -0.10624,1.135 -0.3225,1.64875 -0.21625,0.5175 -0.52999,0.9675 -0.94375,1.36375 -0.42125,0.40375 -0.89374,0.7125 -1.41875,0.925 -0.52499,0.215 -1.06874,0.3225 -1.6375,0.3225 -0.57375,0 -1.12374,-0.11 -1.65125,-0.33125 -0.52749,-0.2225 -0.99624,-0.5375 -1.40875,-0.95125 -0.40875,-0.40625 -0.72,-0.865 -0.93249,-1.37375 -0.21626,-0.50875 -0.3225,-1.045 -0.3225,-1.60375 0,-0.5625 0.10999,-1.1075 0.33499,-1.6325 0.22375,-0.525 0.54625,-0.99375 0.96501,-1.4075 0.40625,-0.4 0.86249,-0.7 1.36999,-0.9025 0.50875,-0.205 1.05751,-0.30375 1.645,-0.30375"
                          fill="#ffffff"
                        />
                        <path
                          d="m 284.01238,20.36463 c -1.37749,-1.02 -3.51499,-1.72375 -6.36,-2.09625 -0.99375,-0.12375 -3.005,-0.28375 -6.14375,-0.48875 -2.0675,-0.12 -3.5725,-0.34 -4.47249,-0.655 -1.18127,-0.405 -1.755,-1.015 -1.755,-1.8625 0,-0.81625 0.54,-1.44875 1.65374,-1.9325 1.1725,-0.50875 2.90625,-0.765 5.145,-0.765 3.4925,0 8.19375,0.5975 12.20875,2.41875 0.115,0.0525 0.205,-0.0463 0.205,-0.225 l 0,-3.73 c 0,-0.18 0.0112,-0.31875 -0.085,-0.36375 -2.555,-1.1725 -8.19,-1.90625 -12.04875,-1.90625 -3.625,0 -6.48125,0.61875 -8.49,1.84125 -2.0525,1.24875 -3.0925,2.9825 -3.0925,5.14875 0,2.12125 0.93001,3.655 2.75751,4.55625 1.51625,0.745 4.16874,1.24625 7.87875,1.4925 4.1375,0.26125 6.69874,0.51375 7.62,0.75 1.68999,0.435 2.50749,1.2625 2.50749,2.52875 0,1.08875 -0.62624,1.9325 -1.91249,2.5775 -1.39876,0.685 -3.42875,1.07375 -5.97,1.035 -3.70501,-0.0588 -8.48375,-0.89375 -12.41501,-3.325 -0.12375,-0.0763 -0.22125,0.002 -0.22125,0.18125 l 0,3.4525 c 0,0.17875 -0.0163,0.33875 0.065,0.3675 0.04,0.015 4.135,3.095 12.54,3.095 3.97375,0 7.07501,-0.65875 9.21625,-1.95875 2.18625,-1.33375 3.2975,-3.23875 3.2975,-5.66875 0,-1.91625 -0.72125,-3.41875 -2.12875,-4.4675 S 339.17225,9.30675 c -0.17874,0 -0.32499,0.14625 -0.32499,0.32625 l 0.055,16.4325 c 10e-4,0.17875 -0.0925,0.21375 -0.20875,0.0763 L 324.78601,9.6493 c -0.11624,-0.13625 -0.22875,-0.27 -0.25125,-0.295 -0.0237,-0.0263 -0.18875,-0.0475 -0.36874,-0.0475 l -4.37375,0 c -0.17875,0 -0.32625,0.14625 -0.32625,0.32625 l 0,21.98375 c 0,0.17875 0.1475,0.325 0.32625,0.325 l 3.6375,0 c 0.17874,0 0.32499,-0.14625 0.32499,-0.325 l 0,-16.99875 c 0,-0.18 0.0962,-0.215 0.21251,-0.0788 l 14.69499,17.15625 c 0.11626,0.13625 0.35875,0.24625 0.5375,0.24625 l 3.61001,0 c 0.17875,0 0.325,-0.14625 0.325,-0.325 l 0,-21.98375 c 0,-0.18 -0.14625,-0.32625 -0.325,-0.32625 l -3.63751,0 z m -29.84149,17.09938 c -1.84625,1.49375 -3.7775,2.25125 -6.875,2.25125 -3.115,0 -5.065,-0.75375 -6.92125,-2.23875 -1.83875,-1.4675 -2.77125,-3.42875 -2.77125,-5.82375 0,-2.315 0.95375,-4.2525 2.83375,-5.76375 1.8975,-1.52375 3.825,-2.3 6.85875,-2.3 2.77499,0 4.5525,0.68125 6.41,2.02 2.13875,1.53875 3.22375,3.57125 3.22375,6.04375 0,2.37625 -0.9275,4.3325 -2.75875,5.81125 m 2.53625,-14.75625 c -2.67875,-2.01875 -5.4625,-3.04375 -9.41125,-3.04375 -3.92125,0 -6.71875,1.02375 -9.4375,3.04375 -3.11625,2.32375 -4.695,5.3325 -4.695,8.945 0,3.67375 1.55874,6.695 4.6325,8.975 2.68124,2 5.4975,3.01125 9.5,3.01125 3.96375,0 6.76375,-1.0025 9.44,-2.98125 3.07375,-2.28125 4.63249,-5.3125 4.63249,-9.005 0,-3.6125 -1.56999,-6.62125 -4.66124,-8.945 M 253.125,26.06575 c 0,0.17875 -0.0925,0.2125 -0.20875,0.0762 L 239.005,9.64945 c -0.115,-0.13625 -0.22749,-0.27 -0.25,-0.295 -0.0238,-0.0263 -0.18875,-0.0475 -0.36874,-0.0475 l -4.37251,0 c -0.17875,0 -0.32499,0.14625 -0.32499,0.32625 l 0,21.98375 c 0,0.17875 0.14625,0.325 0.32499,0.325 l 3.63875,0 c 0.17875,0 0.325,-0.14625 0.325,-0.325 l 0,-17 c 0,-0.17875 0.095,-0.215 0.21125,-0.0775 l 14.69625,17.155 c 0.11625,0.1375 0.35875,0.2475 0.5375,0.2475 l 3.60625,0 c 0.17875,0 0.32625,-0.14625 0.32625,-0.325 l 0,-21.98375 c 0,-0.18 -0.1475,-0.32625 -0.32625,-0.32625 l -3.63875,0 c -0.17875,0 -0.325,0.14625 -0.32375,0.32625 l 0.0588,16.4325 z m -28.38462,5.551 c 0,0.17875 0.14625,0.32625 0.325,0.32625 l 3.63625,0 c 0.17875,0 0.325,-0.1475 0.325,-0.32625 l 0,-21.98375 c 0,-0.17875 -0.14625,-0.32625 -0.325,-0.32625 l -3.63625,0 c -0.17875,0 -0.325,0.1475 -0.325,0.32625 l 0,21.98375 z I 215.79413,17.35 c 0,0.17875 -0.14625,0.32625 -0.32624,0.32625 l -13.23876,0 c -0.17874,0 -0.32499,-0.1475 -0.32499,-0.32625 l 0,-7.7175 c 0,-0.17875 -0.14626,-0.325 -0.325,-0.325 l -3.635,0 c -0.17875,0 -0.32625,0.14625 -0.32625,0.325 l 0,21.985 c 0,0.17875 0.1475,0.325 0.32625,0.325 l 3.635,0 c 0.17874,0 0.325,-0.14625 0.325,-0.325 l 0,-9.75125 c 0,-0.17875 0.14625,-0.325 0.32499,-0.325 l 13.23876,0 c 0.17999,0 0.32624,0.14625 0.32624,0.325 l 0,9.75125 c 0,0.17875 0.145,0.325 0.325,0.325 l 3.63375,0 c 0.17875,0 0.325,-0.14625 0.325,-0.325 l 0,-21.985 c 0,-0.17875 -0.14625,-0.325 -0.325,-0.325 l -3.63375,0 c -0.18,0 -0.325,0.14625 -0.325,0.325 l 0,7.7175 z m -41.94975,-2.53537 c 1.88875,-1.515 4.375,-2.2825 7.3875,-2.2825 3.9875,0 7.04001,1.105 9.3875,3.5425 0.0888,0.0913 0.2,0.0175 0.37,-0.0412 l 3.31,-1.165 c 0.16875,-0.06 0.25,-0.17875 0.18375,-0.265 -3.04374,-3.9975 -7.5725,-5.9975 -13.25125,-5.9975 -3.945,0 -7.3025,1.0125 -9.98125,3.0125 -3.09625,2.3025 -4.66499,5.31125 -4.66499,8.94375 0,3.695 1.54875,6.7275 4.60249,9.0075 2.65751,1.97625 6.03751,2.9825 10.04375,2.9825 2.85375,0 5.33375,-0.4425 7.39501,-1.25375 3.42499,-1.345 5.62374,-4.14625 5.6875,-4.21875 0.0637,-0.0738 -0.0112,-0.205 -0.16876,-0.29 l -3.12124,-1.70375 c -0.15875,-0.085 -0.27126,-0.17625 -0.36251,-0.0738 -1.81124,2.035 -5.26625,3.7225 -9.21249,3.6175 -3.19876,-0.0875 -5.76751,-0.755 -7.635,-2.24 -1.85126,-1.47 -2.78751,-3.43125 -2.78751,-5.8275 0,-2.3125 0.94625,-4.24875 2.8175,-5.7475 m -7.60588,-5.182 c 0,-0.17875 -0.14625,-0.32625 -0.325,-0.32625 l -22.65625,0 c -0.18,0 -0.32624,0.1475 -0.32624,0.32625 l 0,3.21375 c 0,0.18 0.14625,0.325 0.32624,0.325 l 8.86001,0 c 0.17875,0 0.32499,0.1475 0.32499,0.32625 l 0,18.11875 c 0,0.17875 0.14626,0.32625 0.325,0.32625 l 3.63375,0 c 0.17875,0 0.325,-0.1475 0.325,-0.32625 l 0,-18.11875 c 0,-0.17875 0.14626,-0.32625 0.325,-0.32625 l 8.8625,0 c 0.17875,0 0.325,-0.145 0.325,-0.325 l 0,-3.21375 z m -31.27712,13.68275 c 0.0738,1.9225 -0.6975,3.17375 -2.13375,4.04125 -1.32375,0.79875 -2.83749,1.33125 -5.92374,1.33125 -3.08376,0 -4.61251,-0.5325 -5.95376,-1.33125 -1.45875,-0.8675 -2.24375,-2.25 -2.165,-4.04125 l 0,-13.6825 c 0,-0.18 -0.1475,-0.32625 -0.325,-0.32625 l -3.6375,0 c -0.17875,0 -0.32375,0.14625 -0.32375,0.32625 l 0,14.56625 c -0.11625,2.53125 1.18876,4.655 3.53751,6.14625 2.52374,1.59 5.17249,2.26875 8.86749,2.26875 3.69625,0 6.3325,-0.68 8.80625,-2.26875 2.35,-1.47125 3.61376,-3.435 3.54001,-6.14625 l 0,-14.56625 c 0,-0.18 -0.14626,-0.32625 -0.32501,-0.32625 l -3.63874,0 c -0.17876,0 -0.325,0.14625 -0.325,0.32625 l 0,13.6825 z M 106.27963,17.35 c 0,0.17875 -0.14625,0.32625 -0.32499,0.32625 l -13.24001,0 c -0.17875,0 -0.325,-0.1475 -0.325,-0.32625 l 0,-7.7175 c 0,-0.17875 -0.145,-0.325 -0.325,-0.325 l -3.635,0 c -0.17875,0 -0.325,0.14625 -0.325,0.325 l 0,21.985 c 0,0.17875 0.14625,0.325 0.325,0.325 l 3.635,0 c 0.18,0 0.325,-0.14625 0.325,-0.325 l 0,-9.75125 c 0,-0.17875 0.14625,-0.325 0.325,-0.325 l 13.24001,0 c 0.17874,0 0.32499,0.14625 0.32499,0.325 l 0,9.75125 c 0,0.17875 0.14625,0.325 0.325,0.325 l 3.63501,0 c 0.17875,0 0.325,-0.14625 0.325,-0.325 l 0,-21.985 c 0,-0.17875 -0.14625,-0.325 -0.325,-0.325 l -3.63501,0 c -0.17875,0 -0.325,0.14625 -0.325,0.325 l 0,7.7175 z"
                          fill="#ffffff"
                        />
                        <path d="m 71.649,41.26738 1.14626,0 0,-41.265 -1.14626,0 0,41.265 z" fill="#ffffff" />
                        <path
                          d="m 27.15738,40.08775 c 2.0225,1.69 5.26,1.4575 7.27625,-0.5 L 54.28238,19.0065 c 1.30375,-1.5625 1.84875,-6.935 -5.055,-6.95125 -0.135,0 -16.035,0.006 -16.035,0.006 -1.23375,0 -3.09625,0.84375 -4.05875,1.845 l -4.0175,4.18 c -0.45375,0.4675 -0.57125,0.61 -0.485,0.80625 0.1425,0.15125 0.18125,0.12125 0.5925,0.12125 l 12.1325,0.006 c 4.02625,-0.085 4.255,0.20625 4.5225,0.8575 0.15625,0.38125 0.0588,1.24375 -0.975,2.3 l -7.87125,8.0075 c -0.67,0.6775 -1.605,1.06875 -2.57,1.06875 -0.90375,-10e-4 -1.76,-0.3325 -2.405,-0.93625 l -7.17125,-6.645 c -0.66875,-0.6225 -1.0575,-1.475 -1.095,-2.39875 -0.0337,-0.94375 0.3025,-1.84875 0.94625,-2.5375 l 8.33125,-8.95115 c 1.19875,-1.29 3.39375,-2.30125 4.995,-2.30125 l 18.0875,0.009 c 1.24875,0.002 3.15625,-0.795 4.16875,-1.74 l 5.32375,-4.96875 c 0.35125,-0.375 0.5475,-0.63 0.50625,-0.69875 C 62.09858,1e-4 61.97238,0.0126 61.58863,0.0138 L 29.75238,0 c -1.23,0 -3.0575,0.87125 -3.995,1.905 l -15.91,18.0475 c -0.8225,0.9075 -0.78,2.29875 0.09,3.1075 l 17.22,17.0275 z M 0.59375,12.0755 C 0.2,12.0765 0,12.0755 0,12.19175 0,12.358 0.0438,12.4305 0.51876,12.90425 l 4.88499,4.7425 c 0.81375,0.79 2.2175,0.71625 2.985,-0.15375 l 3.845,-4.355 c 0.54375,-0.6975 0.5,-0.765 0.45375,-0.8975 -0.0575,-0.165 -0.20375,-0.16 -0.62,-0.16 L 0.59375,12.0755 z"
                          fill="#E21C2A"
                        />
                      </svg>
                    </div>
                    <h1>%s</h1>
                    <div class='header-accent'></div>
                  </div>

                  <!-- CONTENT -->
                  <div class='content'>
                    <div class='greeting'>%s</div>
                    <div class='message'>%s</div>
                    %s
                    %s
                    %s
                  </div>

                  <!-- FOOTER -->
                  <div class='footer'>
                    <div class='footer-brand'>HUTCHINSON</div>
                    <div class='footer-divider'></div>
                    <div class='footer-text'>
                      © 2025 Hutchinson – Plateforme de gestion des demandes<br>
                      <a href='%s'>Accéder à la plateforme</a>
                    </div>
                  </div>

                </div>
                </div>
                </body>
                </html>
                """;

        return String.format(htmlTemplate,
                title,
                title,
                greeting,
                mainMessage,
                detailsHtml.toString(),
                buttonHtml,
                footerHtml,
                emailService.getBaseUrl()
        );
    }
}